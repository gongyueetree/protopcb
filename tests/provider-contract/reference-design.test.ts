...
describe('Fragment 提取（AD9837 golden 场景，§五十一）', () => {
  ...
  it('depth=1：锚定 AD9837 提取时钟/去耦/滤波/本地电源，连接器排除、MCU 不被整板拖入', () => {
    ...
    // 端口只基于 BFS **实际遍历**的网络：
    //   SPI 三根线把 U2 带进来了，但 U2 的 USB_DP 没有被任何一跳走过 —— 它不是 DDS fragment 的接口。
    //   此前的实现把纳入器件的全部 padNets 都算作 fragment 网络，USB_DP 就被错当成端口（旧断言曾把它当"正确"）。
    const portNames = out.fragment.ports.map((p) => p.name);
    const netNames = out.fragment.nets.map((n) => n.name);
    expect(netNames).not.toContain('USB_DP');
    expect(portNames).not.toContain('USB_DP');
    // 真正走过的：SPI / FSYNC / MCLK / VOUT / 电源 —— 都在
    for (const n of ['SPI_SCLK', 'SPI_SDATA', 'FSYNC', 'MCLK', 'VOUT', '+3V3', 'GND']) expect(netNames).toContain(n);
    // SPI 三根线在 fragment 外还挂着 U2 的其它…不，U2 已纳入；这里 VOUT_FILT 经 R1 走到，C3 在 depth=1 之外 → VOUT_FILT 是边界端口
    expect(portNames).toContain('VOUT_FILT');
    // 整体过 Zod
