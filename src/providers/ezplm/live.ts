/**
 * providers/ezplm/live.ts
 * ezPLM **真实**接口客户端 —— 经 /api/ezplm 签名代理调用（API Key 手册确认的两个只读端点）。
 * EzplmComponentDataProvider（index.ts）的检索能力委托到这里；此前两套客户端并存，
 * 另一套打的 /v1/components/* 从未存在。
 *
 * 官方接口仅两个只读端点：
