...
export function buildKicadPcb(doc: CircuitCanvasDocument): string {
  const L: string[] = [];
  // generator 是**协议标识**（KiCad 用它识别产出方），不是显示品牌：
  // 改名会让既有工程的差异对比出现噪声，故保留 —— 见 docs/ARCHITECTURE.md 的 LEGACY_COMPAT_IDENTIFIER
  L.push(`(kicad_pcb (version 20250114) (generator "circuit_canvas") (generator_version "1.0")`);
  L.push(``);
  L.push(`  (general (thickness 1.6))`);
  L.push(`  (paper "A4")`);
  L.push(`  (title_block (title "${(doc.name || 'protopcb Design').replace(/"/g, '')}") (comment 1 "Generated by Circuit Canvas · ezPLM.cn"))`);
  L.push(``);
  L.push(`  (layers`);
