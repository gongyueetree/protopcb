      ...
      <div style={{ padding: 12, overflow: 'auto', flex: 1 }}>
        ...
        <div style={{ display: 'flex', gap: 4, marginBottom: 10 }}>
          {([['model', '🔍 ' + t('型号搜索')], ['footprint', '📦 ' + t('KiCad封装库')],
            ...
            <button key={id} onClick={() => { setAiGate(null); if (id === 'custom' && !guardCustomPart()) return; setLeftTab(id); }} style={{ flex: 1, padding: '7px 0', fontSize: 12, fontWeight: 700, cursor: 'pointer', border: `1px solid ${leftTab === id ? COLORS.green : '#dbe6dd'}`, borderRadius: 8, background: leftTab === id ? COLORS.greenBg : '#fff', color: leftTab === id ? COLORS.green : '#64748b' }}>{label}</button>
          ))}
        </div>
